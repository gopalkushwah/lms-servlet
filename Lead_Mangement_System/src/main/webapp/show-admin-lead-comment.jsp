...
<%
	...
	int totalLeadCount = LeadDao.getLeadsCountUsingUserId(user.getUserId());
	int totalLeadCountByFacebookSource = LeadDao.getLeadsCountUsingCompanyIdAndSource(user.getUserId(), "facebook");
	int totalLeadCountByGoogleSource = LeadDao.getLeadsCountUsingCompanyIdAndSource(user.getUserId(), "google");
	int employeeCount = UserDao.getEmployeeCountUsingCompanyId(user.getUserId());
%>
<!Doctype HTML>
	<html>
	<head>
...
<div class="row d-flex mt-3">
			<div class="col-9">
				<span class="nav nav-heading"  > Comments </span>
				<span class="nav2 nav-heading"  >  Comments </span>
			</div>
			<div class="col-3">
				<div class="d-flex text-white gap-2 align-items-center">
					<img src="image/xyz.jfif" class="profile-image"/>
					<div class="d-flex flex-column align-items-start ">
						<span class="fs-4"><%= user.getName() %> </span>
						...
						<tr>
							...
							<td>
								<form action ='updateAdminAllLeads.jsp' method='post'>
									<button type='submit' class = 'submit-btn w-100'  name = 'update' value = "<%= leadByLeadId.getLeads_id()%>">Update</button>
								</form>
								<form class="mt-2" action ='DeleteAdminLeads' method = 'post'>
									<button type='submit' class = "submit-btn w-100" name = 'delete' value = "<%= leadByLeadId.getLeads_id() %>" onclick='myFunction()'>Delete</button>
