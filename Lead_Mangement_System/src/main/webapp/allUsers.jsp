...
<%
	...
	int itemsPerPage = 10;
	int userCount = UserDao.getUsersCountUsingCompanyId(userCookie.getUserId());
	int totalPages = (int) Math.ceil((double) userCount / itemsPerPage);
	List<User> listUser = UserDao.getUserByLimitAndCompanyId(userCookie.getUserId(), itemsPerPage, (currentPage - 1) * itemsPerPage);
%>
<!Doctype HTML>
	<html>
	<head>
...
.close{
	...
	<div id="main">
		<div class="row d-flex mt-3">
			<div class="col-9">
				<span class="nav nav-heading"  > All Users</span>
				<span class="nav2 nav-heading"  > All Users</span>
			</div>
			<div class="col-3">
		...
		<div class="pe-2 ps-2">
			<p class="fs-2 text-white box-heading">All Users</p>
		</div>	
		<hr class="divide">
		<div class="main-container">
			<div class="p-2 overflow-x-scroll">
				<% try{String success = session.getAttribute("msg").toString(); 
					...
					<div class='alert alert-danger alert-dismissible fade show'>
						...
						<strong>Danger!</strong> Data deleted successfully 
					</div>
					<%}
			...
			<tr>
				...
				<td>
					<form action ='update-user-admin.jsp?userId=<%=user.getUserId() %>' method = 'post'>
						<button type = 'submit' class = 'submit-btn w-100' name = 'update'>Update</button>
					</form>
					<form class="mt-2" action ='DeleteAdminUsers' method = 'post'>
						<button type = 'submit' class = 'submit-btn w-100' name = 'delete' value = "<%= user.getUserId() %>" onclick='myFunction()'>Delete</button>
					</form>
				</td>
				</tr>	
				<%}%>
