...
<%
	...
	List<Lead> leadList = LeadDao.getAllLeadsByUserId(userCookie.getUserId());
%>
<!Doctype HTML>
	<html>
	<head>
		...
		<tr>	
			...
			<td>
				<form action ='updateMyLead.jsp' method = 'post'>
					<button type = 'submit' class = 'btn btn-primary'  name = 'update' value ="<%=l.getLeads_id() %>">Update</button>
				</form>
			</td>
			<td>
				<form action ='deleteMyLead' method = 'post'>
					<button type = 'submit' class = 'btn btn-danger' name = 'delete' value ="<%=l.getLeads_id() %>" onclick='myFunction()'>Delete</button>
				</form>
			</td>
		</tr>
	<% }%>
