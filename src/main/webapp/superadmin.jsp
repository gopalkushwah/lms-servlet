...
<body>
	...
	<br/><span>Total Leads</span></p>
						...
						<tr>
							...
							<select name = "currentOwner" required>
								...
								<%
										...
										try{
											String email = (String)session.getAttribute("email");
											PreparedStatement pstmt = connect.prepareStatement("select email from users WHERE isadmin=? OR isadmin=? AND issuperadmin=?");			
											pstmt.setString(1, "a");
		...
		<div class="col-12">
			<div class="boxform">
			<%try{
			String already1 = (String)session.getAttribute("already1");
			if(already1 != null){ %>
					<div class='alert alert-success alert-dismissible fade show'>
						<%= already1 %>
					</div>
			<% 
				session.removeAttribute("already1");
				}
			}catch(Exception e){
				System.out.println(e);
			} %>
			<form action = "AddAdminBySuperAdmin" method = "post">
					<table>
						<tr>
							<td>Name </td>
							<td><input type = "text" name = "name" placeholder="Admin name" value="" required/></td>
							<td>Mobile </td>
							<td><input type = "tel" name = "mobile" placeholder="Admin mobile" maxlength= "10" value="" required/></td>
						</tr>
						<tr>
							<td>Password </td>
							<td><input type = "password" name = "password" placeholder="Password" required/></td>
							<td>Email </td>
							<td><input type = "email" name = "email" placeholder="Admin email" required/></td>
						</tr>
						<tr>
							<td>Gender </td>
							<td>
							...
							<td></td>
							<td><button type= "submit" class = "btn">Add Admin</button></td>
						</tr>
						<tr>
						</tr>
					</table>
			...
			<form action = "AddUserBySuperAdmin" method = "post">
					<table>
						<tr>
							...
							<td>Mobile </td>
							<td><input type = "tel" name = "mobile" placeholder="User mobile" maxlength= "10" value="" required/></td>
						</tr>
						<tr>
							<td>Password </td>
							<td><input type = "password" name = "password" placeholder="Password" required/></td>
						...
						<tr>
							...
							<td></td>
							<td><button type= "submit" class = "btn">Add User</button></td>
						</tr>
