					...
					<tr>
						...
						<td>
							<form action ="show-admin-lead-comment.jsp?leadid=<%= l.getId() %>" method = 'post' >
								<button type = 'submit' class = 'btn btn-primary w-100'  name = 'view-comment' value = "<%=l.getEmail() %>">View</button>
							</form>
							<form class="mt-2" action ='add-admin-comment.jsp?leadid=<%= l.getId() %>&useremail=<%= l.getEmail() %>' method = 'post'>
								<button type = 'submit' class = 'btn btn-primary w-100' name = 'add-comment' value = "<%=l.getEmail() %>">Add</button>
							</form>
						</td>
						<td>
							<form action ='updateAdminAllLeads.jsp' method = 'post' >
								<button type = 'submit' class = 'btn btn-primary w-100'  name = 'update' value = "<%=l.getId() %>">Update</button>
							</form>
							<button class = 'btn btn-danger mt-2 w-100' name = 'delete' onclick='myFunction(<%=l.getId() %>)'>Delete</button>
						</td>
					</tr>
					<%}%>
   				</tbody>
...
<!--		Start Pagination -->
			<div>
			    <% if (currentPage > 1) { %>
			        <a class="btn btn-primary" style="padding: 2px 4px; text-decoration: none;" href="/Lead_Mangement_System/allLeads.jsp?page=<%= currentPage - 1 %>"> &lt; Previous</a>
			    <% } %>
			    <% if (currentPage < totalPages) { %>
			        <a class="btn btn-primary" style="padding: 2px 4px; text-decoration: none;" href="/Lead_Mangement_System/allLeads.jsp?page=<%= currentPage + 1 %>">Next &gt;</a>
			    <% } %>
			</div>
			</div>
			</div>
