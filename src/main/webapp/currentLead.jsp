...
<%
	...
	List<Lead> leadList = LeadDao.getLeadsByCurrentOwnerAndCompanyId(userCookie.getEmail(),userCookie.getCompanyId());
%>
<!Doctype HTML>
	<html>
	<head>
...
<script src="https://maxcdn.bootstrapcdn.com/bootstrap/4.1.3/js/bootstrap.min.js"></script>
	...
	<span style="font-size:30px;cursor:pointer; color: white;" class="nav2"  > Current Leads</span>
		...
		<div class="col-div-3">
			<div class="box">
				<p><%= totalLeadCountByStatusFinished %>
				<br/><span>Enrolled</span></p>
		...
		<td><%= l.getSource() %></td>
		<td><%= l.getDate() %></td>
		<td style ="color: rgba(252, 0, 130, 1)"><%= l.getOwner() %></td>
		<% if (l.getPriority().equals("Low")){%>
			<td><center><div class = 'animated-progress progress-green'><span data-progress='33'></span>Low</div></div></center></td>
		<% } if (l.getPriority().equals("High")){%>
			<td><center><div class = 'animated-progress progress-red'><span data-progress='95'></span>High</div></div></center></td>
		<% } if (l.getPriority().equals("Medium")){%>
			<td><center><div class = 'animated-progress progress-purple'><span data-progress='66'></span>Medium</div></div></center></td>
		<% } %>
		<td>
			<form action ='updateMyLead.jsp' method = 'post'>
				<button type = 'submit' name = 'update' class = 'btn btn-primary' value ="<%=l.getLeads_id() %>">Update</button>
			</form>
		<%--
		</td>
			<td>
				<form action ='deleteMyLead' method = 'post'>
					<button type = 'submit' name = 'delete' class = 'btn btn-danger' value="<%=l.getLeads_id() %>" onclick='myFunction()'>Delete</button>
				</form>
			</td>
		</tr>
		 --%> 
